...
BEGIN
    ...
    IF v_id_statusu_oferty != v_id_statusu_oferty_aktywna THEN 
        RAISERROR 23000 'Ta oferta jest zakończona!';
        RETURN;
    END IF;
    ...
    IF EXISTS (
        ...
        RAISERROR 23000 'Agent ma już zaplanowaną wizytę w tym dniu i o tej godzinie! Wybierz inny termin...';
        RETURN;
    END IF;
    ...
    IF EXISTS (
        ...
        RAISERROR 23000 'Klient ma już zaplanowaną wizytę w tym dniu i o tej godzinie! Wybierz inny termin...';
        RETURN;
    END IF;
    
    -- sprawdzam czy oferta ma już jakąś wizytę zaplanowaną na ten dzień i na tą godzinę, właściwie to niepotrzebne, do usunięcia, bo sprawdzenie agenta i klienta to załatwia
    IF EXISTS (
        SELECT 1
        FROM wizyta
        WHERE wizyta.id_oferty = p_id_oferty AND wizyta.data_wizyty = p_data_wizyty AND wizyta.godzina_wizyty = p_godzina_wizyty
    ) THEN
        RAISERROR 23000 'Oferta ma już zaplanowaną wizytę w tym dniu i o tej godzinie! Wybierz inny termin...';
        RETURN;
    END IF;
